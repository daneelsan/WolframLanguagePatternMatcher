...
\textbf{MExpr-level optimizations} can be performed on the pattern AST before bytecode generation.
The MExpr representation preserves full pattern structure, enabling transformations like constant folding (\texttt{f[1+2]} → \texttt{f[3]}), pattern simplification (\texttt{x\_ | x\_} → \texttt{x\_}), and common subpattern factoring.
