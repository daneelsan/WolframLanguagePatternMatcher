...
In Wolfram Language, everything is an expression with the form \texttt{head[arg1, arg2, ...]}. For example:
\begin{itemize}
    \item \texttt{f[x, y]} has head \texttt{f} and two arguments
    \item \texttt{5} is equivalent to \texttt{Integer[5]}
    \item \texttt{\{a, b\}} is equivalent to \texttt{List[a, b]}
\end{itemize}

Patterns describe classes of expressions by specifying constraints on heads, arguments, and structure.
...
Patterns define function behavior based on argument structure:
\begin{mmaCell}{Input}
fac[0] := 1
fac[n_] := n * fac[n - 1]
fac[5]
...
Multiple definitions with patterns enable dispatch by structure:
\begin{mmaCell}{Input}
...
\begin{enumerate}
    ...
    \item \textbf{Observability}: Expose execution state for debugging and analysis
\end{enumerate}
