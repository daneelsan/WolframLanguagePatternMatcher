...
% ============================================================================
\begin{abstract}
Pattern matching is fundamental to symbolic computation in the Wolfram Language, yet its current implementation—based on recursive interpretive evaluation—introduces significant limitations for complex patterns. Patterns involving alternatives, deep nesting, and user-defined predicates incur not only high runtime cost under repeated evaluation but also opaque control flow and backtracking behavior, which hinder analysis, debugging, and optimization.
...
\begin{enumerate}
    \item A minimal instruction set (~20 opcodes) tailored to pattern-matching primitives in the Wolfram Language.
...
\texttt{x\_} &
Named pattern that binds the matched expression \\
\texttt{x\_ \dots x\_} &
