...
pub const c = struct {
    ...
    /// typedef int mbool;
    pub const wlr_mbool = isize;

    /// typedef int64_t mint;
    pub const wlr_mint = isize;
    /// typedef unti64_t umint;
    pub const wlr_umint = usize;

    pub const wlr_TRUE = 1;
    pub const wlr_FALSE = 0;
    ...
    pub extern fn wlr_SameQ(firstExpression: wlr_expr, secondExpression: wlr_expr) wlr_mbool;
    // mint wlr_Length(wlr_expr expression)
    pub extern fn wlr_Length(expression: wlr_expr) wlr_mint;
    // WLR_ATTRIBUTE wlr_expr wlr_Part(wlr_expr expression, mint index);
...
pub const Expr = union(ExprTag) {
    ...
    pub fn part(self: Expr, index: isize) !Expr {
        // TODO: expand to support array-type expression, or association
        return switch (self) {
            .Normal => {
    ...
    pub fn sameQ(self: Expr, expr: Expr) bool {
        const res = c.wlr_SameQ(self, expr);
        return res == c.wlr_TRUE;
    }

    pub fn length(self: Expr) usize {
        const len = c.wlr_Length(self);
        if (len == -1) {
